...
This doesn't just apply to prompts and tooling — it applies to architecture too. There's a growing argument that AI works better with monoliths. A single coherent codebase where the model can see the domain model, the data flows, and the internal APIs all at once. In a microservice architecture, context is fragmented across repositories, services, message queues, and infrastructure layers. So should we restructure our systems to be more AI-friendly?

I'd argue no. Models change faster than we change our projects. Imagine refactoring any of your current microservice architectures into a monolith. That's infrastructure changes, testing pipelines, deployment strategies, documentation migrations. The organisational lift is enormous — and by the time you've finished, the models have moved on. The models will adapt to our current architecture faster than we'll restructure our systems to suit the models. Each new model already handles cross-repo context and fragmented codebases better than the last. Restructuring your architecture to optimise for today's model limitations is just scaffolding at a larger scale.

The tooling is already catching up. In Claude Code, you can run `/add-dir` to pull in as many repositories as you need for the problem you're solving. Working across three microservices for a feature? Add all three. The model gets the full picture without you needing to collapse everything into a single repo. The fragmentation problem is being solved at the tool layer, not the architecture layer.

Here's a practical example of smaller-scale scaffolding. You don't need to run `/init` on your projects. You don't need an elaborate CLAUDE.md, COPILOT.md, or AGENTS.md. The model can read your `package.json`. It can figure out your directory structure. It can work out your test command. Most of what people put in these files is information the agent can already discover and auto-generated context files actually *reduce* task success while increasing cost. You're adding noise.
...
So if most of what we build around AI tools is temporary — whether it's prompt chains, context files, or entire architecture migrations — what actually compounds? Clear specifications. Good test suites. Well-structured codebases. The prompt that gets your current model to handle edge cases correctly — that's scaffolding. The acceptance criteria that define what "correct" actually means — that's foundation. One has a shelf life of weeks. The other compounds over time.

If you're spending more time tuning your AI workflow than clarifying what you're actually trying to build, you might be investing in the wrong layer. The models will meet you where you are.
